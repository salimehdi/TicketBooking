...
function Cart() {
  ...
  const loadInfo = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/api/cart`, { // Use a relative URL
        headers: headers,
      });
  ...
  async function removeFromCart(nameOfEventToBeRemoved) {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/api/removefromcart/${nameOfEventToBeRemoved}`, { headers: headers });
                  ...
                  {cartItems.map((item, index) => (
                    <div style={{ color: "#fff" }} key={index} className="cart">
                      ...
                      <div className="exploreAndRemoveBtn">
                        ...
                        <button onClick={()=>{navigate(`/explore/${item.name}`)}}>Explore</button>
                      </div>
                    </div>
                  ))}
                </div>
